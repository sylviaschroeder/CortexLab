function error = threeVarsNonvisConstrained(respTraining, varsTraining, ...
...
% initial estimtate of linear dependence of nonvisual signal on intercepts
b =  [ones(length(x0{1}),1), x0{1}] \ x0{2};
x0{2} = b;
x0 = cat(1, x0{:});
...
if all(terms == 1) % full model
    x1 = x([stim, numStim+1, numStim+2, numStim+2+stim, end]);
elseif terms(1) == 1 % no interaction
    x1 = [x([stim, numStim+1, numStim+2, numStim+2+stim]); 0];
else % no nonvis
    x1 = [x([stim, numStim+1, numStim+2]); 0; 0];
end
...
for iStim = 1:numStim
    ...
    if all(terms == 1) % full model
        pars = parameters([iStim, numStim+1, numStim+2, numStim+2+iStim, end]);
    elseif terms(1) == 1 % no interaction
        pars = [parameters([iStim, numStim+1, numStim+2, numStim+2+iStim]); 0];
    else % no baseline
        pars = [parameters([iStim, numStim+1, numStim+2]); 0; 0];
    end
...
estimate = parameters(1) + ...
    (parameters(2) + parameters(3) * parameters(1)) * nonvis + ...
    parameters(4) * baseline + parameters(5) * nonvis .* baseline;