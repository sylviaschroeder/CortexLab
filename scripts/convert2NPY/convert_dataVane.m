...
%% Folders
folderBase = 'C:\Users\Sylvia\OneDrive - University of Sussex\Projects\2021_Vane_Columns';
folderROIData = fullfile(folderBase, 'RawData');
...
for k = 1:length(db)
    ...
    if str2double(subject(end-1:end)) > 50 % boutons
        folderSave = fullfile(folderBase, 'DataToPublish\boutons');
    else
        folderSave = fullfile(folderBase, 'DataToPublish\neurons');
    end
