...
folderScript = 'C:\dev\workspace\CortexLab';
folderData = 'Z:\UCLData\Ephys_Task\Subjects';

folderShared = 'C:\Users\Sylvia\OneDrive - University of Sussex\Projects\Ephys_task_left and right SC\Data';
...
alfDir = fullfile(fullfile(folderShared, mouseName, date));
% if ~exist(alfDir, 'dir')
%    mkdir(alfDir)
% end
...
% % write timeline time
% writeNPY(t, fullfile(alfDir, '_ss_signals.timestamps.npy'))
% % write wheel signal
% wheelPos = wheel.correctCounterDiscont(tl.rawDAQData(:, ...
%     strcmp({tl.hw.inputs.name}, 'rotaryEncoder')))';
% % convert to cm: divide by resolution (1024*4), multiply by circumference
% % (diameter*pi, diameter = 62mm), resolution and diameter according to
% % mouseInput.EncoderResolution and .WheelDiameter in 
% % \\ZSERVER.cortexlab.net\Code\Rigging\config\ZGOOD\hardware(2018-04-13).mat
% wheelPos = wheelPos ./ (1024 * 4) .* (6.2 * pi); % in cm
...
% [moveOn, moveOff, moveDispl, peakVelTime, peakAmp] = wheel.findWheelMoves3( ...
%     wheelPos, t, Fs, 'posThresh', 0.02, 'tThresh', 0.2, ...
%     'posThreshOnset', 0.001);
% writeNPY([moveOn moveOff], fullfile(alfDir, '_ss_wheelMoves.intervals.npy'));
% writeNPY(moveDispl, fullfile(alfDir, '_ss_wheelMoves.displacement.npy'));
% writeNPY(peakVelTime, fullfile(alfDir, '_ss_wheelMoves.peak_times.npy'));
% writeNPY(peakAmp, fullfile(alfDir, '_ss_wheelMoves.amplitude.npy'));
...
%% write time of face video frames
% load face video frame times
filename1 = fullfile(root, num2str(db(k).expTL), 'face_timeStamps.mat');
...
%     addpath('C:\STORAGE\workspaces\Rigbox\cortexlab')
%     addpath(genpath('C:\STORAGE\workspaces\Rigbox\cb-tools'))
    preproc.alignVideo(mouseName, date, db(k).expTL, 'face');
end

if isfile(filename1)
    d = load(filename1);
    writeNPY(applyCorrection(d.tVid, tlToMaster), ...
        fullfile(alfDir, 'face.timestamps.npy'));
end
clear d
