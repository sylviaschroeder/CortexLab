...
for k = 1:length(db)
    ...
    units_OT = find(any(depths >= db(k).OTdepth(:,1)' & depths <= ...
        db(k).OTdepth(:,2)', 2));
    
    for iCell = 1:length(units_good)
        % get spiketimes for this cell
        st = sp.st(sp.clu == units_good(iCell));
        ...
        hold on
        thisTemp = squeeze(sp.tempsUnW(templates(units_good(iCell)+1)+1,:,:));
...
%         spatialRFs = RF';
        ...
        annotation('textbox', [.47 .95 .17 .04], 'String', sprintf(...
            'Neuron %d (good unit %d)', units_good(iCell), iCell), ...
            'FontSize', 15, 'FontWeight', 'bold', ...
            'LineStyle', 'none', 'HorizontalAlignment', 'left')
        ind = find(units == units_good(iCell));
