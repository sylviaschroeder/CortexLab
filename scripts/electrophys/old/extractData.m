...
for k = 1:length(db)
    ...
    tlFile = dat.expFilePath(db(k).subject, date, db(k).tlExp, 'timeline', 'master');
%     tlFile = fullfile(expInfo, db(k).subject, date, db(k).tlExp, ...
%         sprintf('%s_1_%s_Timeline.dat', date, db(k).subject));
    protocolFile = dat.expFilePath(db(k).subject, date, db(k).exp, 'parameters', 'master');
    ...
    stimTimes.onset = stimOn;
    stimTimes.offset = stimOff;
