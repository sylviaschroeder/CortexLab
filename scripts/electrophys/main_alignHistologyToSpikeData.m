...
folder = 'C:\STORAGE\OneDrive - University College London\Lab\RESULTS\OpticTract\tracedPoints_histology_CCFatlas';
...
save(fullfile(folder, sprintf('%s_%s_%s_tracedPoints.mat', subject, date, tag)), ...
    'tracedPoints')
% OR LOAD POINTS ----------------------------------------------------------
if isempty(tag)
    load(fullfile(folder, sprintf('%s_%s_tracedPoints.mat', subject, date)))
else
    load(fullfile(folder, sprintf('%s_%s_%s_tracedPoints.mat', subject, date, tag)))
end
% -------------------------------------------------------------------------
