...
% drifitng gratings
for k=1:length(drifting)
    folder = [folderROIData filesep drifting(k).subject filesep ...
        drifting(k).date filesep num2str(drifting(k).exp)];
    fileStart = [drifting(k).date '_' num2str(drifting(k).exp) '_' ...
        drifting(k).subject];
    file = [fileStart '_2P_plane%03d_ROI.mat'];
    clear info
    for iPlane=1:length(drifting(k).planes)
        % load meta
        data=load(fullfile(folder, sprintf(file,drifting(k).planes(iPlane))));
        m = orderfields(data.meta);
        info(iPlane) = m;
    end
    titleLine = [drifting(k).subject ', ' drifting(k).date ', exp. ' ...
        num2str(drifting(k).exp) ', planes ' num2str(drifting(k).planes)];
