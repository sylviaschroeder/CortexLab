function [receptiveFields, runKernels, runWin, ...
    ...
    getReceptiveField_smooth_running(traces, traceTimes, ...
...
if shiftTest && fitSeparate % shiftTest not implemented when fitting together
    ...
    fprintf('  Lambdas (of %d) to get expl. var. of RF on shifted traces: \n', length(lamStim))
    for lam = 1:length(lamStim)
        fprintf('    Lam %d: ',lam)
        for m = 1:length(models)
            fprintf('m%d ',m)
            indCells = find(bestLams(:,m) == lam & valid);
            shTraces = NaN(size(residuals,1), length(indCells), numShifts);
            for sh = 1:numShifts
                shTraces(:,:,sh) = circshift(residuals(:,indCells), shifts(sh), 1);
            end
            for fold = 1:crossFolds
%                 reset(gDev);
                fprintf('.')
...
%                 A = [st_train; lamMatrix_stim .* lamStim(lam)];
                A = [st_train; gpuArray(lamMatrix_stim .* lamStim(lam))];
                tr = reshape(cat(1, shTraces(j,:,:), ...
                    zeros(size(stim,2), size(shTraces,2), numShifts)), ...
                    sum(j)+size(stim,2), []);
                numBatches = ceil(size(tr,2) / batchSize);
%                 tr = gpuArray(tr);
                
                pred = NaN(sum(~j), size(tr,2));
                for b = 1:numBatches
                    % B = A \ tr;
                    c = (b-1)*batchSize+1 : min(b*batchSize,size(tr,2));
                    B = gather(A \ gpuArray(tr(:,c)));
                    pred(:,c) = st_test * B;
                end
                pred = reshape(pred, sum(~j), length(indCells), numShifts);
                explainedVariance_stimOnly_shifted(indCells,fold,m,:) = ...
                    1 - sum((shTraces(~j,:,:)-pred).^2,1) ./ ...
                    sum((shTraces(~j,:,:)-mean(shTraces(j,:,:),1)).^2,1);
            end
            fprintf(' ')
        end
        fprintf('\n')
