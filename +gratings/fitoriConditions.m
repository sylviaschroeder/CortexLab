...
fixedPars = NaN(1,5);
% ind = find(all(paramLimits(2:end,:) == ...
%     repmat(paramLimits(1,:),size(paramLimits,1)-1,1), 1));
% fixedPars(ind) = paramLimits(1,ind);
% if ismember(3, ind)
%     if ismember(2, ind)
%         fixedPars(3) = (1-fixedPars(3))/(1+fixedPars(3)) * fixedPars(2);
%     else
%         fixedPars(3) = NaN;
%     end
% end
if isempty(parsInit)
    parsInit = fitoriWrapped(oo, rr, [], fixedPars, '', n);
...
for k = 1:10
    ...
    [ err, pars ] = fitit(  'gratings.orituneWrappedConditions', rr,...
