...
folders = {'\\zserver.cortexlab.net\Data\expInfo', ...
    '\\zubjects.cortexlab.net\Subjects'};
file = fullfile(info.subject, info.expDate, num2str(info.exp), ...
    sprintf('%s_%d_%s_Timeline.mat', info.expDate, info.exp, info.subject));
...
% end
try
    data = load(fullfile(folders{1}, file));
catch
    data = load(fullfile(folders{1}, file));
end
...
if ~isfield(Timeline, 'ballUDPCount')
    ballData = [];
    return
end
